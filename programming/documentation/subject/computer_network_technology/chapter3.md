...
***
  ...
  + TCP
    ...
    - 数据偏移（首部长度）：占4个bit。它指出TCP报文段的数据起始处距离TCP报文段的起始处有多远。数据偏移的单位是4字节，4bit可以表示0～15，所以首部长度最多15x4Byte=60Byte。
  ...
  + 慢启动和拥塞避免
    ...
    - 更新后的ssthresh值变为12（即发送窗口数值24的一般），拥塞窗口重新设置为1，并执行慢启动算法。
