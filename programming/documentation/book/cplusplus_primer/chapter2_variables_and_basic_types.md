...
## 基本内置类型
  ...
  + C++定义了一套包括算术类型和空类型在内的基本数据类型。
    ...
    - ![](./resources/basic_data_types.png)
...
extern int i; // 声明
int j;        // 定义
extern double pi = 3.1416; // 定义，因为不能初始化
```
...
## 处理类型
  ...
  + 类型别名
    ...
    - 如果某个类型别名指代的是复合类型或常量，那么把它用到声明语句里就会产生错误的理解，如下面第二个示例。
...
```
  ...
  + auto类型说明符：
    ...
    - 复合类型、常量和auto
      ...
      - 编译器推断一个引用类型时，auto会忽略引用，如下面第一个示例。

      - 编译器推断一个常量时，auto一般会忽略掉顶层const，同时底层const则会保留下来，如下面第二个示例。

      - 编译器推断一个常量时，auto用符号&或*修饰，则不会忽略掉顶层const，如下面第三个示例。
...
```
  ...
  + decltype类型指示符
    ...
    - decltype处理顶层const和引用的方式与auto有些许不同，它会保留顶层const和引用，如下面第一个示例。

    - 复合类型、括号和decltype

      - 编译器推断一个表达式时，引用作为右值的计算可能会导致decltype忽略引用，如下面第二个示例。

      - 编译器推断一个表达式时，因为解引用指针可以得到指针所指的对象，而且还能给这个对象赋值，导致decltype的结果是引用类型，如下面第二个示例。

      - 编译器推断一个表达式时，如果给变量外加上一层或多层括号，变量将作为一种赋值语句左值的特殊表达式，导致decltype的结果是引用类型，如下面第三个示例。

```c++
const int ci = 0, &rci = ci;
decltype(ci) x = 0; // An integer constant
decltype(cj) y = x; // A reference to an interger constant
```
