# 数据结构和算法
  ...
  + [将序列分解为单独的变量](#1将序列分解为单独的变量)

  + [从任意长度的可迭代对象中分解元素](#2从任意长度的可迭代对象中分解元素)

  + [保存最后n个元素](#3保存最后n个元素)

  + [找到最大或最小的n个元素](#4找到最大或最小的n个元素)

  + [实现优先级队列](#5实现优先级队列)

  + [在字典中将键映射到多个值上](#6在字典中将键映射到多个值上)

  + [让字典保持有序](#7让字典保持有序)

  + [与字典有关的计算问题](#8与字典有关的计算问题)

  + [在两个字典中寻找相同点](#9在两个字典中寻找相同点)

  + [从序列中移除重复项且保持元素间顺序不变](#10从序列中移除重复项且保持元素间顺序不变)

  + [对切片命名](#11对切片命名)

  + [序列中出现次数最多的元素](#12序列中出现次数最多的元素)

  + [通过某个关键字排序一个字典列表](#13通过某个关键字排序一个字典列表)

  + [排序不支持原生比较的对象](#14排序不支持原生比较的对象)

  + [通过某个字段将记录分组](#15通过某个字段将记录分组)

  + [过滤序列元素](#16过滤序列元素)

  + [从字典中提取子集](#17从字典中提取子集)

  + [映射名称到序列元素](#18映射名称到序列元素)

  + [转换并同时计算数据](#19转换并同时计算数据)

  + [合并多个字典和映射](#20合并多个字典和映射)
...
***
  ...
  + N比集合数量小很多时，heapq.nlargest heapq.nsmallest
...
***
  ...
  + 字典的keys()和values()方法返回keys-view对象和items-view对象，支持集合操作。
...
# 使用_replace()方法会创建一个全新的命名元组并将对应的字段用新的值取代
...
***
  ...
  + 生成器表达式
    ...
    - 生成器表达式作为一个单独参数传递给函数时可以不用多加一个括号。
